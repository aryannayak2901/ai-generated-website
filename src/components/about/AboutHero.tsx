...
export function AboutHero({ className, tag, title, subtitle, image }: AboutHeroProps) {
  ...
  return (
    <section className={`relative w-full min-h-[60vh] flex items-center bg-charcoal-primary overflow-hidden ${className || ""}`}>
      {/* Gradient background */}
      ...
      <div className="w-full max-w-[1280px] mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center px-6 md:px-8 lg:px-12 py-20 lg:py-32 relative z-10">
        ...
        >
          <div className="inline-flex items-center gap-3 mx-auto lg:mx-0 mb-3">
            ...
            <span className="text-teal-primary font-bold tracking-[0.3em] uppercase text-[10px] md:text-xs">
              {tag || "Our Legacy"}
            </span>
          </div>
          ...
          <h1 className="font-serif text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-4 leading-tight tracking-tight">
            {title || (
              <>A Tradition of <span className="text-teal-primary">Legal Excellence</span>.</>
            )}
          </h1>
          
          <div className="w-24 h-0.5 bg-teal-primary/50 mx-auto lg:mx-0 mb-4" />
        ...
        >
          ...
          />
          <div className="absolute inset-0 bg-gradient-to-tr from-charcoal-primary/40 via-transparent to-transparent pointer-events-none" />
        </motion.div>
      </div>
    </section>
  );
