      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="max-w-5xl mx-auto text-center flex flex-col items-center animate-fade-in-up">
          <div className="inline-flex items-center gap-3 px-6 py-2.5 mb-10 text-[10px] font-bold tracking-[0.3em] uppercase rounded-full bg-gold/10 text-gold border border-gold/20 backdrop-blur-md shadow-lg shadow-gold/5 animate-scale-in [animation-delay:200ms]">
            ...
            {tag || "Contact Chambers of Jeet Bhatt"}
          </div>

          <h1 className="font-serif text-5xl md:text-8xl lg:text-9xl font-bold text-white mb-10 leading-[0.95] tracking-tight animate-fade-in-up [animation-delay:400ms]">
            {title ? title : (
              <>Schedule Legal <span className="text-gold italic font-medium">Consultation</span></>
            )}
          </h1>

          <p className="text-xl md:text-3xl text-slate-300 font-sans leading-relaxed max-w-3xl mx-auto mb-16 font-light animate-fade-in-up [animation-delay:600ms]">
            {subtitle ? subtitle : (
              <>Whether you&apos;re facing a complex legal challenge or seeking strategic counsel, we provide <span className="text-white font-medium">unmatched expertise</span> with absolute integrity.</>
            )}
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-6 md:gap-8 w-full max-w-2xl mx-auto animate-fade-in-up [animation-delay:800ms]">
            <Button
