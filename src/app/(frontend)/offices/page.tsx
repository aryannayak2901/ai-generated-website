        ...
        if (layoutBlock.blockType === "officeHero") {
          ...
          return (
            <div key={index} className="max-w-5xl mx-auto text-center flex flex-col items-center">
              <div className="inline-flex items-center gap-3 px-6 py-2.5 mb-10 text-[10px] font-bold tracking-[0.3em] uppercase rounded-full bg-accent/10 text-accent border border-accent/20 backdrop-blur-md shadow-lg shadow-accent/5">
                <span className="w-2 h-2 rounded-full bg-accent animate-pulse" />
                {heroBlock.tag || "Distinguished Chambers"}
              </div>
              
              <h1 className="text-4xl md:text-6xl lg:text-7xl font-serif text-foreground mb-10 leading-[0.95] tracking-tight">
                {heroBlock.title ? heroBlock.title : (
                  <>Our <span className="relative inline-block">
                    <span className="text-accent italic font-medium">Chambers</span>
                  </span></>
                )}
              </h1>
              
              <p className="text-lg md:text-xl text-muted-foreground font-sans leading-relaxed max-w-3xl mx-auto">
                {heroBlock.subtitle ? heroBlock.subtitle : (
                  <>Strategically located at the heart of Gujarat&apos;s legal landscape, providing <span className="text-foreground font-medium"> unmatched expertise</span> and accessibility.</>
                )}
              </p>
            </div>
          );
